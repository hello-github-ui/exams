...
的,通过 Future-Listener 机制,用户可以方便的主动获取或者通过通知机制获得IO 操作结果。

#### 8.1.2. Netty 高性能
在 IO 编程过程中,当需要同时处理多个客户端接入请求时,可以利用多线程或者IO 多路复用技术
...
## 3. Netty 的文件传输采用了 transferTo 方法,它可以直接将文件缓冲区的数据发送到目标 Channel,
...
表面上看,串行化设计似乎 CPU 利用率不高,并发程度不够。但是,通过调整NIO 线程池的线程
...
## 2. 将 处 理 结 果 的 回 调 对 象 callback , 存 放 到 全 局 ConcurrentHashMap 里面
...
requestID , 再 从 前 面 的 ConcurrentHashMap 里面 get(requestID) , 从 而 找 到
...
于实现远程过程调用的应用程序编程接口。它使客户机上运行的 程序可以调用远程服务器上的对象。远
...
生成引擎可以在多种语言中,如 C++, Java, Python, PHP, Ruby, Erlan g, Perl, Haskell, C#, Cocoa,
