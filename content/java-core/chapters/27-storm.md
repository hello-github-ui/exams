...
##### 27.1.1.4. Task
worker 中每一个spout/bolt 的线程称为一个task. 在 storm0.8 之后,task 不再与物理线程对应,
...
##### 27.1.2.2. Spout
在一个 topology 中获取源数据流的组件。通常情况下spout 会从外部数据源中读取数据,然后转
换为 topology 内部的源数据。
...
随机分组,尽量均匀分布到下游 Bolt 中将流分组定义为混排。这种混排分组意味着来自Spout 的
...
不关注并行处理负载均衡策略时使用该方式,目前等同于shuffle grouping,另外 storm 将会把
